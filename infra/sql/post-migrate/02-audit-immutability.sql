-- Audit immutability, enforced by the database itself. Applied after every migration, and
-- safe to re-run.
--
-- Two independent mechanisms, because they fail differently:
--
--   The REVOKE below denies UPDATE and DELETE to the application role. It is targeted at
--   audit_event specifically — the earlier version did it with a default-privilege REVOKE,
--   which stripped UPDATE and DELETE from every future table in the schema.
--
--   The trigger denies them to *everyone*, including a superuser session opened by an
--   administrator with good intentions and a bad idea. Together with the hash chain, that is
--   what makes the trail evidence rather than a log
