...
| [Phase 6.6 — Electronic Signature & Witnessed Attestation UI](./reports/phase-6.6-electronic-signature-ui.md) | **BLOCKED**, and why that is the right outcome rather than a failure. The signature domain honours all nine of ADR-0017's decisions and is the most carefully built thing in the product; what it rests on does not exist. Rate limiting is specified in two architecture documents — `15 §7` has a five-row table, `17` repeats it — and `@nestjs/throttler` is installed and imported nowhere, `RATE_LIMIT_*` is parsed and read by nothing, the four global guards are auth, tenancy, RBAC and ACL, and no edge config supplies it; so the endpoint that takes a password and answers one deliberately-undifferentiated refusal is an unbounded online password oracle aimed at exactly the stolen-session attacker §11.200 re-authentication exists to stop. And the schema comment promising "one live signature per person, revision and purpose, partial on `withdrawn_at`" describes an index that was never created: the three declarations beneath it are `@@index`, the migration emits three plain `CREATE INDEX`, and transactions run at READ COMMITTED — so two concurrent requests can both pass the existence check and mint two attestations of the same fact. Also the finding that there is no signature *request* model at all: no pending, expired or cancelled state, no recipient — a signature is taken, not requested, so the brief's request UI had no backend and inventing one would have meant inventing states. Both blockers are pre-existing, neither touches the signature domain, and about three days of work between them unblocks a ceremony that needs no new endpoint | **Re-opened after Phase 6.7 unblocked it, and BLOCKED again on a different, smaller gap:** the ceremony must show the exact canonical attestation statement before the user confirms, and no route returns it before the signature exists. `statementBody` appears on the verification response alone, which needs a `signatureId`; `serialiseSignatureStatement` runs only inside `sign`, from values the server resolves at that instant. Rendering it in the browser is forbidden and wrong — ADR-0017 stores the bytes verbatim so verification never regenerates them — signing first inverts the ceremony, and paraphrasing discards the §11.50 manifestation that is the evidence. The smallest additive fix is a preview route behind the same permission and scope, which belongs to a small backend phase rather than a UI one |
...
| [Phase 6.7 — Rate Limiting Completion (Part A)](./reports/phase-6.7-rate-limiting-completion.md) | The rule table that had sat unread since Phase 5, finally wired: a guard registered ahead of authentication — because sign-in must be limited by address before an identity exists — counting through the existing `CachePort`, whose Redis adapter is `INCR` plus `EXPIRE NX` in one `MULTI` and therefore atomic across connections. Proven where it matters: two guards over two independent adapters with two Redis connections, requests alternating between them like a load balancer, stopping at one shared budget — a test a process-local `Map` would fail. The tenant-isolation test earned its place by finding a real defect in that key design: only the tenant dimension was namespaced, so `rl:{rule}:identity:{user}` was a global bucket two tenants could share and one customer could use to deny service to another. Also why `auth.password-reset` is retained as UNUSED rather than deleted — no such route exists, and creating one to satisfy a catalogue is backwards — why the login key is the submitted address never resolved to an account, and why the status stays PARTIALLY COMPLETE with every test passing: the proof stops at the guard rather than traversing the real HTTP pipeline, and accepting it would mean accepting exactly the standard Phase 6.6 refused for the previous two controls || [Phase 6.7 — Rate Limiting Completion (Part A)](./reports/phase-6.7-rate-limiting-completion.md) | **COMPLETE.** The rule table that had sat unread since Phase 5, wired into a guard registered ahead of authentication — because credential stuffing is unauthenticated by definition — counting through the existing `CachePort`, whose Redis adapter is `INCR` plus `EXPIRE NX` in one `MULTI` and therefore atomic across connections. Proven at two levels, and the second level is the point: nine guard tests prove the mechanism (two independent adapters sharing one budget, tenant isolation, concurrency, fail-closed on Redis loss), and a booted Nest application proves a real HTTP request reaches it — six `POST /api/v1/documents/:id/signatures` calls where the first five are refused by authorization and the sixth by the limiter with `429`, a shape that can only occur if the guard runs first. Writing that last test was not a formality: it found that the route table never matched a real path, because `configureApp` mounts everything under `/api` and the matcher stripped only the version — **every rule would have been inert in production while all nine guard tests reported green**. The third time this sequence has found a control that was declared, configured and unreachable, after Phase 6.3's phantom permissions and Phase 6.4's unread `DeliveryState.FAILED`. Both Phase 6.6 blockers are now closed |
